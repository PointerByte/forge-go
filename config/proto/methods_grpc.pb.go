...
func (c *greeterClient) CreateChat(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[ChatMessage, ChatSummary], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[0], Greeter_CreateChat_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ChatMessage, ChatSummary]{ClientStream: stream}
	return x, nil
}
...
func (c *greeterClient) StreamAlerts(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AlertMessage, AlertMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[1], Greeter_StreamAlerts_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AlertMessage, AlertMessage]{ClientStream: stream}
	return x, nil
}
...
func (UnimplementedGreeterServer) CreateChat(grpc.ClientStreamingServer[ChatMessage, ChatSummary]) error {
	return status.Error(codes.Unimplemented, "method CreateChat not implemented")
}
func (UnimplementedGreeterServer) StreamAlerts(grpc.BidiStreamingServer[AlertMessage, AlertMessage]) error {
...
func _Greeter_CreateChat_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreeterServer).CreateChat(&grpc.GenericServerStream[ChatMessage, ChatSummary]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Greeter_CreateChatServer = grpc.ClientStreamingServer[ChatMessage, ChatSummary]

func _Greeter_StreamAlerts_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreeterServer).StreamAlerts(&grpc.GenericServerStream[AlertMessage, AlertMessage]{ServerStream: stream})
}
