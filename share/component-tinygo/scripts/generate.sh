...
# from `share/tinygo-wasip2`, which already fetched them under checksum verification.
set -Eeuo pipefail
...
if [[ ! -x "${tools_dir}/wasm-tools" || ! -x "${tools_dir}/wkg" ]]; then
  ...
  # verification the centralized PoC already established, then reuse the exact same binaries.
  tinygo_poc="${root}/../tinygo-wasip2"
  "${tinygo_poc}/scripts/fetch-tools.sh"
  mkdir -p "${tools_dir}"
  install -m 0755 "${tinygo_poc}/artifacts/tools/wasm-tools" "${tools_dir}/wasm-tools"
  install -m 0755 "${tinygo_poc}/artifacts/tools/wkg" "${tools_dir}/wkg"
fi

if [[ ! -d "${root}/wit/deps" ]]; then
  # The reviewed WASI 0.2.0 dependency tree lives with the centralized PoC for the same reason.
...
(
  ...
  PATH="${tools_dir}:${PATH}" \
    ...
    go run go.bytecodealliance.org/cmd/wit-bindgen-go@v0.7.0 generate \
      --world goforge \
      --out generated \
      --package-root github.com/PointerByte/forge-go/share/component-tinygo/generated \
