...
tar \
  ...
  --exclude='./component-tinygo/artifacts/goforge.component.wasm' \
  -cf "${input_archive}" \
  -C "${repo}" \
  ./portable ./component ./component-tinygo
...
docker run --rm -i \
  ...
  sh -c '
    ...
    tinygo version >&2
    PATH=/tmp/src/component-tinygo/artifacts/tools:$PATH tinygo build \
      -target=wasip2 \
      -o artifacts/goforge.component.wasm \
      --wit-package wit/pointerbyte-goforge-0.1.0.wasm \
      --wit-world goforge \
      . >&2
    tar -cf - -C /tmp/src/component-tinygo artifacts/goforge.component.wasm
