...
- **Misma lógica de guest.** `guest.go` importa el paquete de producción `share/component/bridge` en lugar
...
- **Misma superficie exportada.** `scripts/check-world.sh` compara la interfaz `operations` de
  `wit/world.wit` contra `share/component/wit/world.wit` y falla ante cualquier desviación.
...
`share/tinygo-wasip2`, que los descargó con verificación de checksum.

## Estado

Esto es evidencia de investigación que respalda un cambio de toolchain de producción **propuesto**.
No se ha aplicado: `share/component/scripts/build.sh` y `openspec/metadata/toolchain.yaml` siguen
