# forge-go API Inventory

This standard-library-only command generates the Phase 0 declaration catalog used by the
forge-go/forge-deno functional coverage matrix. It scans all six workspace modules while excluding
generated research, isolated WASI comparison tools, OpenSpec, vendor, coverage, and test-data directories.

Run it reproducibly from the forge-go repository root:

```bash
cd share/api-inventory
...
module so it cannot alter forge-go's product coverage denominator.
